...
- Alertmanager Webhook集成, 告警信息支持以短信方式进行发送至对应项目负责人
- 告警分析看板(T+1调度), 便于对告警做分析
...
| 告警分析看板功能发布, 之前考虑将数据写到Hudi再同步到Hive实现湖仓一体,但目前暂时先写到Hive,之后再集成到Hudi |

| 下一阶段事项                                                 |
| ------------------------------------------------------------ |
| Airflow Dag整理及拆分<br>湖仓一体集成,Hudi的数据存储使用HDFS<br>Terraform调研和集成 |
